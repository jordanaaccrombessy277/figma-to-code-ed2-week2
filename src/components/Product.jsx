...
function Product({id, productImg, handleBuyNow, productPrice, productCurrencyCode, productTitle, cart}) {
        ...
        <div className="w-full group h-auto relative">
            ...
            <div className="absolute hidden rounded-32 bg-blackopacity23 top-0 w-full h-full group-hover:flex flex-wrap justify-center items-end">
                <div className="flex flex-row gap-1 pb-4">
                    <Link to={`/product-details/${encodeURIComponent(id)}`} className="uppercase font-jetbrainsmono font-extrabold md:text-sm text-xs flex flex-row gap-1.5 px-4 py-2.5 bg-white rounded-3xl items-center"> <img src={cart} alt="" /> <span className='mt-0.5'>Add to cart</span></Link>
                    <button onClick={()=>handleBuyNow()} className="uppercase font-jetbrainsmono font-extrabold md:text-sm text-xs px-10 py-2.5 border border-white text-white rounded-3xl">Buy now</button>
                </div>
            </div>
        </div>
